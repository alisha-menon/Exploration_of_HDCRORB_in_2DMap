...
class game_module:
    ...
    def train_from_file_NN(self, filename):
        self.hd_module.train_from_file_NN(filename)
        self.num_cond = self.hd_module.num_cond
        self.num_thrown = self.hd_module.num_thrown
        self.num_cond_state1 = self.hd_module.num_cond_state1
        self.num_thrown_state1 = self.hd_module.num_thrown_state1
...
# *****************************************************************************
    ...
    def autoplay_game(self, gametype, NN):
        pygame.init()
        screen = pygame.display.set_mode(self.pixel_dim)
        clock = pygame.time.Clock()
        running = True
        ...
        while running:
            ...
            while not_crash:
                ...
                if NN:
                    act_out = self.hd_module.test_sample_NN(current_sensor)
                elif self.hd_module.two_states:
    ...
    def play_game_from_file(self, test_filename,goal_filename, NN, count):
        ...
        for i in range(count):
            ...
            stu.append(stuck)
            t.append(end-start)
            ste.append(np.mean(np.array(step_count)))
            xstu.append(x_stuck_count)
            ystu.append(y_stuck_count)
    ...
    def test_game(self, num_test, NN):
        not_crash = True
...
# *********************** CHANGE BASED ON SENSOR DATA *************************
    def get_sensor(self):
        ...
        # sensor_vals.extend([math.atan2(delta_y, delta_x), np.sqrt(np.power(delta_x, 2) + np.power(delta_y, 2))])
        return sensor_vals
# *****************************************************************************
