    ...
    for g = 1:numGests
        for trial = 1:numTrials
            ...
            for i = 1:testLength-oldModel.N+1
                ...
                ngram = ngram_iscas(segment, oldModel);
                for c = 1:numNewTests
                    for d = 1:numOldConfigs
                        [sims(c + (d-1)*numNewTests,i,:), outLabel(c + (d-1)*numNewTests,i)] = classify_iscas_fast(ngram, testAM{testIdx(c),d});
                    end
                end
            end
            ...
            for i = 1:testLength-oldModel.N+1
                ...
                for c = 1:numOldTests
                    for d = 1:numNewConfigs
                        [sims(c + (d-1)*numOldTests,i,:), outLabel(c + (d-1)*numOldTests,i)] = classify_iscas_fast(ngram, testAM{d,testIdx(c)});
                    end
                end
            end
