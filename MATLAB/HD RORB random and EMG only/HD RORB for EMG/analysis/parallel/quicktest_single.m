...
%     updatecontext_func(model,gestures,trainFeatures,testFeatures,1,1,outfname);
%     
...
% %         jobs(end+1) = parfeval(p,@crossvalidate_func,0,model,gestures,features,Nshot,outfname);
%         crossvalidate_func(model,gestures,feature,Nshot,outfname);
%     end
    ...
    testFeatures = get_features(testData, model.period, featureFunc);
    for n_sh = Nshift
        outfname = [fileDes '-Rotate_-' num2str(n_sh)];
%         jobs(end+1) = parfeval(p,@rotated_func,0,model,gestures,trainFeatures,testFeatures,n_sh,outfname);
        rotated_func(model,gestures,trainFeatures,testFeatures,Nshot,n_sh,outfname);
    end
