function [] = all_experiments_parallel_sub(sub)
...
outputDir = ['./parallel/outputs/' runtime '/'];
mkdir(outputDir)
...
for ii = 1:numRuns
    ...
    gestures = singleDOF;
    trainData = exp{subject}{7};
    testData = exp{subject}{9};
    trainFeatures = get_features(trainData, model.period, featureFunc);
    testFeatures = get_features(testData, model.period, featureFunc);
    for n_sh = Nshift
        outfname = [fileDes '-Rotate_-' num2str(n_sh)];
        jobs(end+1) = parfeval(p,@rotated_func,0,model,gestures,trainFeatures,testFeatures,Nshot,n_sh,outfname);
    end
